...
class PaymentGateway extends AbstractGateway
{
    ...
    public function getDb()
    {
    	...
    	if($conn) {
            $config = $this->config['paymentMongoDB'];
            $dbname = $config['uri_options']['authSource'];
    ...
    public function updateTransaction($data)
    {
    	...
    	try {
    		...
    		if($trans) {
                ...
                // log
                $this->getServiceManager()->get('PassportService')->getMapper()->log($data['user']['id'], $data, $collectionName);
    	...
    	try {
	    	...
	    	if($res) {
	    	    ...
	    	    return ['code' => 1, 'result' => [
                        ...
                        'winner_balance' => $winnerBalance['balance'],
                        'loser_balance' => $loserBalance['balance']
                    ]
                ];
            }
    	}catch(\Exception $e) {
